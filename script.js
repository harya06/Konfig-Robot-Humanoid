...
document.addEventListener("DOMContentLoaded", function () {
	...
	const hcomInput = document.getElementById("hcom");
	...
	const hcomShell = document.getElementById("hcomShell");
	...
	const hcomError = document.getElementById("hcomError");

	const fieldKcom = document.getElementById("fieldKcom");
	const fieldHcom = document.getElementById("fieldHcom");
	const hcomModeRadios = document.getElementsByName("hcomMode");
	...
	const hcomCalcNote = document.getElementById("hcomCalcNote");
	...
	const summaryHcom = document.getElementById("summaryHcom");
	...
	// ====== TOGGLE MODE HCOM ======
	function toggleHcomMode() {
		const mode = document.querySelector('input[name="hcomMode"]:checked').value;

		if (mode === "kcom") {
			fieldKcom.classList.remove("field-hidden");
			fieldHcom.classList.add("field-hidden");
			hcomInput.value = "";
		} else {
			fieldKcom.classList.add("field-hidden");
			fieldHcom.classList.remove("field-hidden");
			kcomInput.value = "0.51";
		}
	...
	hcomModeRadios.forEach((radio) => {
		radio.addEventListener("change", toggleHcomMode);
	});

	// ====== VALIDASI INPUT ======
	function resetErrors() {
		[htopShell, hheadShell, hlegShell, kcomShell, hcomShell].forEach((el) => {
			if (el) el.classList.remove("invalid");
		});
		[htopError, hheadError, hlegError, kcomError, hcomError].forEach((el) => {
	...
	function validateInputs() {
		...
		if (mode === "kcom") {
			kcom = toNumber(kcomInput.value);
		} else {
			hcom = toNumber(hcomInput.value);
		}
		...
		if (hhead === null) {
			...
			if (hhead < minHead || hhead > maxHead) {
				valid = false;
				hheadShell.classList.add("invalid");
				hheadError.textContent = `Hhead harus di antara ${minHead.toFixed(2)} cm dan ${maxHead.toFixed(2)} cm.`;
			}
		}
		...
		if (mode === "kcom") {
			...
			if (hcom === null) {
				valid = false;
				hcomShell.classList.add("invalid");
				hcomError.textContent = "Masukkan nilai Hcom.";
			} else if (hcom <= 0) {
				valid = false;
				hcomShell.classList.add("invalid");
				hcomError.textContent = "Hcom harus lebih besar dari 0.";
			} else if (htop != null && hcom > htop) {
				valid = false;
				hcomShell.classList.add("invalid");
				hcomError.textContent = "Hcom tidak boleh melebihi Htop.";
			}
		}

		return { valid, htop, hhead, hleg, kcom, hcom, mode };
	}

	// ====== PERHITUNGAN RUMUS ======
	function hitung() {
		const { valid, htop, hhead, hleg, kcom, hcom, mode } = validateInputs();
		if (!valid) {
		...
		if (outHcom) outHcom.textContent = fmt(finalHcom, 2, " cm");
		...
		// Update chip dan note berdasarkan mode
		if (mode === "kcom") {
			if (chipKcom) chipKcom.innerHTML = `k<sub>com</sub> = ${finalKcom.toFixed(2)}`;
			if (hcomCalcNote) hcomCalcNote.innerHTML = `H<sub>com</sub> = k<sub>com</sub>·H<sub>top</sub>`;
		} else {
			if (chipKcom) chipKcom.innerHTML = `k<sub>com</sub> = ${finalKcom.toFixed(2)} (dihitung)`;
			if (hcomCalcNote) hcomCalcNote.innerHTML = `H<sub>com</sub> (input manual)`;
		}
		...
		if (summaryHcom) summaryHcom.textContent = fmt(finalHcom, 2, " cm");
	...
	function resetForm() {
		...
		// Reset ke mode kcom dengan nilai default 0.51
		if (kcomInput) kcomInput.value = "0.51";

		// Set radio button ke kcom
		const kcomRadio = document.querySelector('input[name="hcomMode"][value="kcom"]');
		if (kcomRadio) kcomRadio.checked = true;

		toggleHcomMode();
		resetErrors();

		const outputs = [outHtop, outHhead, outHleg, outHcom, outWspread, outWdown, outAfoot, outLarm, outMmax, summaryHtop, summaryHcom, summaryHeadRatio, summaryHleg];
		outputs.forEach((el) => {
			if (el) el.textContent = "–";
		});
	...
	function downloadCSV() {
		...
		try {
			// Header CSV
			const headers = ["Parameter", "Nilai", "Satuan", "Waktu Perhitungan"];
			...
			const rows = [
				...
				["Mode Perhitungan Hcom", calculatedData.hcomMode === "kcom" ? "Rasio k_com" : "Input Manual", "-", ""],
